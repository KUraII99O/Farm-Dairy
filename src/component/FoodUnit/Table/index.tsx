...
interface FoodUnitTableProps {
  sortedFoodUnits: FoodUnit[];
  handleSort: (key: string) => void;
  sortIcon: (key: string) => React.ReactNode;
  handleEditDrawerOpen: (foodUnit: FoodUnit) => void;
  handleDeleteConfirmation: (id: string) => void;
  translate: (text: string) => string;
  formClass: string;
}

const FoodUnitTable: React.FC<FoodUnitTableProps> = ({
  sortedFoodUnits,
  handleSort,
  sortIcon,
  ...
  <Table>
    ...
    <Table.Body className="divide-y">
      {sortedFoodUnits.map((foodUnit, index) => (
        <Table.Row
          key={foodUnit.id}
        ...
        >
          ...
          <Table.Cell>{foodUnit.name}</Table.Cell>
          <Table.Cell>
            <div className="flex items-center">
              <button
