...
const EditFoodUnitForm = ({ foodUnit, onSubmit, onClose }) => {
  const { id } = useParams();
      ...
      <div className="fixed inset-0 overflow-y-auto z-50 flex justify-end">
        <div className="w-96 bg-white h-full shadow-lg p-6" ref={formRef}>
          ...
          <form onSubmit={handleSubmit}>
            <h2 className="text-xl font-bold mb-4">{foodUnit ? "Edit Food Unit" : "Add New Food Unit"}</h2>
            <div className="mb-4">
              <label htmlFor="name" className="block text-sm font-medium text-gray-700">Food Unit Name * :</label>
              <input
                type="text"
                name="name"
                value={formData.name}
            ...
            <div className="flex justify-end">
              ...
              >
                {foodUnit ? "Update Food Unit" : "Add Food Unit"}
              </button>
            </div>
          </form>
        </div>
