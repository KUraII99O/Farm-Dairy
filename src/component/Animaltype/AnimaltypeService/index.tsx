...
async function fetchAnimalTypes(): Promise<AnimalType[]> {
  ...
  try {
    const response = await fetch('http://localhost:3000/animal-types?userId=' + user.id);
    if (!response.ok) {
...
async function addAnimalType(newAnimalType: Omit<AnimalType, 'id' | 'userId'>): Promise<AnimalType> {
  ...
  try {
    ...
    const response = await fetch('http://localhost:3000/animal-types', {
      method: 'POST',
      headers: {
...
async function editAnimalType(id: string, updatedAnimalType: Omit<AnimalType, 'id'>): Promise<void> {
  try {
    const response = await fetch(`http://localhost:3000/animal-types/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
