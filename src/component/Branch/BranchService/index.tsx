...
async function fetchBranches(): Promise<Branch[]> {
    ...
    try {
        const response = await fetch(`https://auth-api-woad.vercel.app/api/branches?userId=${user.id}`);
        if (!response.ok) {
...
async function addBranch(newBranch: Omit<Branch, 'id' | 'userId'>): Promise<Branch> {
    ...
    try {
        ...
        const response = await fetch('https://auth-api-woad.vercel.app/api/branches', {
            method: 'POST',
            headers: {
...
async function editBranch(id: string, updatedBranch: Omit<Branch, 'id'>): Promise<void> {
    try {
        const response = await fetch(`https://auth-api-woad.vercel.app/api/branches/${id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
            },
