...
async function fetchSuppliers(): Promise<Supplier[]> {
  ...
  try {
    const response = await fetch('https://auth-api-woad.vercel.app/api/suppliers?userId=' + user.id);
    if (!response.ok) {
...
async function addSupplier(newSupplier: Omit<Supplier, 'id' | 'userId'>): Promise<Supplier> {
  ...
  try {
    ...
    const response = await fetch('https://auth-api-woad.vercel.app/api/suppliers', {
      method: 'POST',
      headers: {
...
async function editSupplier(id: string, updatedSupplier: Omit<Supplier, 'id'>): Promise<void> {
  try {
    const response = await fetch(`https://auth-api-woad.vercel.app/api/suppliers/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
