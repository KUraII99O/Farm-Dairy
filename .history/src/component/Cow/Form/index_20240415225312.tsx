...
const EditCowForm = () => {
  ...
  const MAX_FILE_SIZE_BYTES = 10 * 1024 * 1024; // Maximum allowed file size (10MB in bytes)
  ...
  const handleFileChange = async (e) => {
    ...
    if (file.size > MAX_FILE_SIZE_BYTES) {
      ...
      // Handle error: File size exceeds the maximum allowed size
      return;
    }
  
    const options = {
      maxSizeMB: 1, // Maximum file size after compression (1MB)
      maxWidthOrHeight: 1024, // Maximum width or height of the image (1024px)
      useWebWorker: true, // Use web workers for faster compression
    };
  
    try {
      const compressedFile = await imageCompression(file, options);
      const blobURL = URL.createObjectURL(compressedFile); // Create blob URL for preview
