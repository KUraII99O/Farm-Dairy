  ...
  const handleSubmit = (e) => {
    ...
    image.onload = function () {
      if (image.width !== 1024 || image.height !== 1024) {
        // Display error message if dimensions are not 1024x1024
        console.error("Image dimensions should be 1024x1024 pixels.");
      } else {
        // Proceed with form submission if dimensions are correct
        if (isEditMode) {
          editCow(parseInt(id), formDataToSend)
            .then(() => navigate("/manage-cow"))
            .catch((error) => console.error("Error submitting form:", error));
        } else {
          addCow(formDataToSend)
            .then(() => navigate("/manage-cow"))
            .catch((error) => console.error("Error submitting form:", error));
        }
      }
