...
const StallList: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (selectedStall) {
      handleUpdateStall(formData);
    } else {
      handleAddNewStall(formData);
    }
  };

  const [formData, setFormData] = useState<Stall>({
    id: selectedStall ? selectedStall.id : 0,
    stallNumber: selectedStall ? selectedStall.stallNumber : "",
    details: selectedStall ? selectedStall.details : "",
    status: selectedStall ? selectedStall.status : true, // Ensure default status is set to "true" for available
  });
  ...
  useEffect(() => {
    setFormData({
      ...
      stallNumber: selectedStall ? selectedStall.stallNumber : "",
      details: selectedStall ? selectedStall.details : "",
      status: selectedStall ? selectedStall.status : true,
    });
  }, [selectedStall]);
